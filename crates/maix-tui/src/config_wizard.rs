...
impl ConfigWizard {
    ...
    fn handle_key(&mut self, key: KeyEvent) {
        match key.code {
            ...
            KeyCode::Esc => {
                // Allow empty config, proceed anyway
                self.done = true;
            }
            _ => {}
        }
    ...
    pub fn save_config(&self) -> std::io::Result<PathBuf> {
        ...
        let content = format!(
            r#"# Maix-Agent config (auto-generated by quick-setup)
[providers.{provider}]
api_key = "{api_key}"
api_base = "{api_base}"
model = "{model}"
...
fn render_wizard(f: &mut Frame, wizard: &ConfigWizard) {
    ...
    render_field(f, chunks[0], "Provider", &wizard.provider, wizard.active_field == 0);
    render_field(f, chunks[1], "API Key", &masked_key(&wizard.api_key), wizard.active_field == 1);
    render_field(f, chunks[2], "API Base", &wizard.api_base, wizard.active_field == 2);
    render_field(f, chunks[3], "Model", &wizard.model, wizard.active_field == 3);

    let hint = if wizard.active_field == 3 {
        "Press Enter to finish, Esc to skip"
    } else {
        "Tab to switch fields, Enter to confirm"
    };

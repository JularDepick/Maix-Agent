...
```
启动:
  TUI模式 → terminal-kit初始化 → 渲染UI → 等待输入
  Backend模式 → 加载配置 → 初始化Provider/Memory/Tools → 启动API服务

用户输入:
  输入 → 命令检测(/开头) → 命令处理
       → 普通输入 → 构建上下文 → 调用Provider → 流式输出

Agent主循环(MAX_TOOL_ROUNDS轮):
  调用Provider → 解析响应 → 文本输出/工具调用 → 工具执行 → 结果回传 → 继续循环
                                                      ↓
                                                 审批检查(非自动批准工具) → 等待用户决策

工具调用:
  工具注册表查询 → 风险级别检查 → 自动批准判断 → 执行工具 → 返回结果
```
...
- 默认系统提示词，索引：`src/backend/agent/agent.ts:57-65`
- Token 估算系数（4字符/token），索引：`src/backend/agent/context.ts:12`、`src/backend/provider/base.ts:23`
...
# 辅助脚本
<!-- 主要指放在工作目录 `assist-scripts/` 文件夹内的脚本文件,是由Agent撰写并维护的,需要按需撰写,辅助脚本的存在主要是为了补充Agent技能/工具能力的不足、减轻批量工作时的上下文负担、供给GitHub工作流自动化调用,主要功能包括但不限于:
- 测试与罗列可用的开发环境
- 执行快捷命令的组合和扩展
- 执行文件内容的检索与替换
- 获取与解析网络页面的内容
...
- 自动生成变更日志与更新文档: 在Release发布时,自动根据 Conventional Commits 规范生成 CHANGELOG.md,并自动构建和部署项目文档(如 GitHub Pages)至指定分支

...
const outDir = path.join(__dirname, '..', 'build', 'bunbuild-backend-solo-windows_x64');
const esbuildScript = path.join(__dirname, 'esbuild-backend-solo.mjs');
...
const bundlePath = path.join(__dirname, '..', 'dist', 'esbuild-backend-solo', 'index.mjs');
...
const targets = [
  { name: 'maix-agent-backend-win-x64.exe', target: 'bun-windows-x64' },
];
...
console.log('\nUsage:');
console.log('  maix-agent-backend-win-x64.exe');
...
const outDir = path.join(__dirname, '..', 'build', 'bunbuild-tui-shell-windows_x64');
const esbuildScript = path.join(__dirname, 'esbuild-tui-shell.mjs');
...
const bundlePath = path.join(__dirname, '..', 'dist', 'esbuild-tui-shell', 'index.mjs');
...
const targets = [
  { name: 'maix-agent-tui-win-x64.exe', target: 'bun-windows-x64' },
];
...
console.log('\nUsage:');
console.log('  maix-agent-tui-win-x64.exe');